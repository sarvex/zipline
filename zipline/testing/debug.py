...
def debug_mro_failure(name, bases):
    ...
    output_file = f'{name}.dot' if os.environ.get('DRAW_MRO_FAILURES') else None
...
def add_direct_edges(g, child, bases):
    ...
    # class declaration.
    g.add_path([b.__name__ for b in bases], label=f'{child.__name__}(O)')

    # Add direct edges.
    for base in bases:
        g.add_edge(child.__name__, base.__name__, label=f'{child.__name__}(D)')
...
def add_implicit_edges(g, child, bases):
    ...
    for base in bases:
        g.add_path([b.__name__ for b in base.mro()], label=f'{base.__name__}(L)')
